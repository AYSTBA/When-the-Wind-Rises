...
export default function SocialSystemFaqPage() {
  const followTargets = [
    ...
    { title: "关注标签", description: "你可以围绕某个主题长期追踪相关风笺。" },
    { title: "关注风笺", description: "适合需要长期跟进回复、开奖、采纳或更新的单个风笺。" },
  ]

  const blockEffects = [
    "你拉黑对方后，无法继续关注对方；如果对方拉黑了你，你也无法关注对方。",
    "双方存在拉黑关系时，不能再互相发起或发送私信。",
    "双方存在拉黑关系时，不能继续在对方风笺下回复，也不能在回复链里继续互动。",
    ...
    >
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 text-sm leading-7 text-muted-foreground md:grid-cols-3">
          ...
          <div className="rounded-xl bg-secondary/40 p-4">后续双方也不能再互相关注、发私信，或继续在对方风笺下互动。</div>
        </CardContent>
      </Card>
    </FaqPageFrame>
  )
