...
const ADMIN_MODULE_SEARCH_ITEMS: AdminModuleSearchItem[] = [
  { href: "/admin", label: "总览", category: "后台", description: "查看用户、风笺、举报和趋势总览。", keywords: ["首页", "概览", "数据面板", "统计", "趋势", "控制台", "后台首页", "仪表盘"] },
  { href: "/admin?tab=users", label: "用户管理", category: "模块", description: "查看用户资料、状态、风铃和登录记录。", keywords: ["用户", "账号", "昵称", "改昵称", "修改昵称", "昵称修改", "昵称价格", "禁言", "封禁", "风铃", "用户风铃", "会员用户", "用户状态"] },
  { href: "/admin?tab=posts", label: "风笺管理", category: "模块", description: "筛选风笺、审核内容、置顶和精华。", keywords: ["风笺", "发笺", "风笺审核", "审核风笺", "精华", "置顶", "关闭回复", "开放回复", "下线", "下线风笺", "风笺下线", "离线风笺", "推荐风笺"] },
  ...
  { href: "/admin?tab=structure", label: "版块管理", category: "模块", description: "维护分区、节点和发笺权限。", keywords: ["版块", "分区", "节点", "导航", "发笺权限", "板块", "栏目", "风广场结构"] },
  ...
  { href: getAdminSettingsHref("vip", "tasks"), label: "任务系统", category: "模块", description: "配置任务条件、周期、状态和风铃奖励。", keywords: ["任务", "任务系统", "任务中心", "每日任务", "新手任务", "挑战任务", "签到任务", "风铃任务", "任务奖励", "task"] },
  ...
  { href: "/admin?tab=reports", label: "举报中心", category: "模块", description: "处理风笺、回复和用户举报。", keywords: ["举报", "投诉", "违规", "风控", "举报处理"] },
  { href: "/admin?tab=logs", label: "日志中心", category: "模块", description: "查看管理员、登录、风铃、上传、支付和 VIP 订单日志。", keywords: ["日志", "操作记录", "登录日志", "风铃日志", "上传日志", "支付流水", "审计日志"] },
  { href: "/admin?tab=logs&logSubTab=admin", label: "管理员日志", category: "日志子页", description: "查看管理员处理举报、审核、封禁、置顶等后台操作记录。", keywords: ["管理员日志", "后台日志", "操作日志", "审核日志", "封禁日志", "禁言日志", "风笺审核日志"] },
  ...
  { href: "/admin?tab=logs&logSubTab=points", label: "风铃日志", category: "日志子页", description: "查看风铃收入、支出和各类风铃变动记录。", keywords: ["风铃日志", "风铃记录", "风铃变动", "风铃收入", "风铃支出", "扣风铃", "加风铃"] },
  { href: "/admin?tab=logs&logSubTab=uploads", label: "上传日志", category: "日志子页", description: "查看头像、风笺图片和附件上传记录。", keywords: ["上传日志", "上传记录", "图片上传日志", "头像上传日志", "附件上传日志", "风笺图片上传"] },
  ...
  { href: "/admin?tab=logs&logSubTab=orders", label: "VIP 订单日志", category: "日志子页", description: "查看 VIP 购买、风铃支付和时长变更记录。", keywords: ["VIP订单日志", "VIP订单", "购买记录", "付款记录", "会员订单"] },
  ...
  { href: "/admin/apps/ai-reply", label: "AI 助手后台", category: "应用设置", description: "配置 AI 开关、模型接口、提示词、代理账号和异步回复任务。", keywords: ["ai", "大模型", "智能回复", "ai 助手", "代理账号", "提示词", "模型接口", "base url", "api key", "评论机器人", "自动回笺", "@ai"] },
  { href: "/admin/apps/gobang", label: "五子棋应用后台", category: "应用设置", description: "配置五子棋免费次数、门票风铃、AI 难度和获胜奖励。", keywords: ["五子棋", "gobang", "五子棋后台", "五子棋设置", "每日普通免费次数", "每日 VIP 免费次数", "普通用户每日总次数", "VIP 用户每日总次数", "门票风铃", "超额门票风铃", "AI 难度", "获胜奖励", "前台入口名称"] },
  ...
  { href: "/admin/apps/payment-gateway", label: "支付网关后台", category: "应用设置", description: "管理支付网关基础配置、充值套餐、支付提供方、通道启停和场景路由规则。", keywords: ["支付网关", "支付后台", "充值套餐", "支付路由", "支付通道", "支付提供方", "payment gateway", "订单超时", "自定义充值", "风铃充值"] },
  ...
  { href: getAdminSettingsHref("profile", "homepage"), label: "首页展示", category: "站点设置子页", description: "配置风笺链接模式、首页 feed、左右侧栏展示和搜索开关。", keywords: ["首页展示", "风笺链接显示模式", "slug模式", "id模式", "首页风笺列表形式", "画廊模式", "右侧统计卡片", "站点公告", "站内搜索", "用户主页 IP 归属地", "IP归属地", "归属地", "左侧导航", "全局左侧导航模式", "左侧导航显示模式", "树状展示", "左侧展示模式", "隐藏左侧导航", "吸附左侧导航", "左侧默认隐藏"] },
  ...
  { href: getAdminSettingsHref("registration", "invite"), label: "注册与邀请码", category: "站点设置子页", description: "控制注册开关、邀请码策略和邀请奖励。", keywords: ["注册与邀请码", "允许新用户注册", "显示邀请码输入框", "注册必须邀请码", "风铃购买邀请码", "初始注册赠送风铃", "邀请人奖励", "被邀请人奖励"] },
  ...
  { href: getAdminSettingsHref("interaction"), label: "互动与热度", category: "站点设置", description: "配置打赏、热度、评论和互动规则。", keywords: ["互动", "热度", "打赏", "评论", "点赞", "回笺", "回复"] },
  { href: getAdminSettingsHref("interaction", "access"), label: "访问控制", category: "站点设置子页", description: "配置游客是否必须登录后才能浏览风广场内容。", keywords: ["访问控制", "游客访问", "登录后浏览", "必须登录", "游客浏览", "风广场内容", "浏览风广场内容", "登录可见"] },
  ...
  { href: getAdminSettingsHref("interaction", "content-limits"), label: "内容限制", category: "站点设置子页", description: "配置发笺标题、正文、回复的字数限制，以及风笺和评论的可编辑时长。", keywords: ["内容限制", "标题最小字数", "标题最大字数", "正文最小字数", "正文最大字数", "回复最小字数", "回复最大字数", "字数限制", "风笺可编辑分钟数", "评论可编辑分钟数", "编辑时效", "编辑分钟数"] },
  { href: getAdminSettingsHref("interaction", "anonymous-post"), label: "匿名发笺", category: "站点设置子页", description: "配置匿名发笺开关、价格、次数和匿名回复身份规则。", keywords: ["匿名发笺", "匿名回复", "匿名马甲", "匿名发笺价格", "每日匿名发笺次数", "匿名帖默认匿名回复"] },
  { href: getAdminSettingsHref("interaction", "tipping"), label: "打赏送礼", category: "站点设置子页", description: "配置风笺打赏开关、次数限制、礼物列表和打赏送礼税。", keywords: ["打赏送礼", "风笺打赏", "每日可打赏次数", "单帖可打赏次数", "裸风铃打赏档位", "礼物打赏", "打赏税", "送礼税", "税率BPS"] },
  { href: getAdminSettingsHref("interaction", "gates"), label: "发布门槛", category: "站点设置子页", description: "配置发笺、评论的邮箱验证、手机验证和注册时长门槛。", keywords: ["发布门槛", "发笺门槛", "评论门槛", "邮箱验证门槛", "手机验证门槛", "验证手机", "绑定手机", "注册时长门槛", "注册分钟数", "发笺限制", "评论限制"] },
  { href: getAdminSettingsHref("interaction", "reward-pool"), label: "红包与聚宝盆", category: "站点设置子页", description: "配置风笺红包、随机命中概率、聚宝盆风铃池和中奖概率。", keywords: ["红包与聚宝盆", "风笺红包", "红包最大风铃", "每日发红包风铃上限", "红包随机命中概率", "聚宝盆", "初始风铃", "回复递增风铃", "聚宝盆回复中奖概率"] },
  { href: getAdminSettingsHref("interaction", "heat"), label: "热度算法", category: "站点设置子页", description: "配置风笺热度权重、阈值和颜色色板。", keywords: ["热度算法", "浏览权重", "回复权重", "点赞权重", "打赏次数权重", "打赏风铃权重", "热度阈值", "热度颜色", "颜色色板"] },
  { href: getAdminSettingsHref("interaction", "preview"), label: "热度预览", category: "站点设置子页", description: "实时预览热度分数和颜色阶段表现。", keywords: ["热度预览", "热度分数", "颜色预览", "浏览数", "回复数", "点赞数", "打赏次数", "打赏风铃"] },
  ...
  { href: getAdminSettingsHref("vip", "redeem-codes"), label: "兑换码管理", category: "站点设置子页", description: "在风铃与VIP设置里管理兑换码生成和兑换。", keywords: ["兑换码", "兑换", "redeem", "礼品码", "卡密"] },
  { href: getAdminSettingsHref("vip"), label: "风铃与VIP", category: "站点设置", description: "管理风铃名称、VIP 价格和会员权益。", keywords: ["风铃", "vip", "会员", "vip1", "vip2", "vip3", "会员权益", "vip价格", "风铃费用", "风铃规则", "补签", "补签价格", "补签费用", "签到", "签到奖励", "签到奖励数量", "签到风铃", "改名价格", "改昵称", "修改昵称", "昵称价格", "下线风笺", "风笺下线", "下线费用", "邀请码价格", "购买邀请码价格"] },
